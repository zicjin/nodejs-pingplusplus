...
- (void)login:(CDVInvokedUrlCommand *)command {
  ...
  // self.appId = [[[NSBundle mainBundle] infoDictionary]
  // objectForKey:@"SPEECH_APP_ID"];

  //    [self.commandDelegate runInBackground:^{
  NSString *initString =
      [[NSString alloc] initWithFormat:@"appid=%@", self.appId];
...
- (void)startListening:(CDVInvokedUrlCommand *)command {
  ...
  NSDictionary *options =
      [command.arguments objectAtIndex:0 withDefault:[NSNull null]];
  //    [self.commandDelegate runInBackground:^{
  if (!self.recognizer) {
...
- (void)startSpeaking:(CDVInvokedUrlCommand *)command {
  ...
  NSDictionary *options =
      [command.arguments objectAtIndex:1 withDefault:[NSNull null]];
  ...
  if (!self.synthesizer) {
    ...
    [self.synthesizer
        setParameter:@"50"
              forKey:[IFlySpeechConstant SPEED]]; //合成的语速,取值范围 0~100
    [self.synthesizer
        setParameter:@"80"
              forKey:[IFlySpeechConstant VOLUME]]; //合成的音量;取值范围 0~100
    [self.synthesizer
        setParameter:@"vixr"
              forKey:[IFlySpeechConstant VOICE_NAME]]; //发音人,默认为”xiaoyan”

    [self.synthesizer
        setParameter:@"8000"
              forKey:[IFlySpeechConstant
                             SAMPLE_RATE]]; //音频采样率,目前支持的采样率有
                                            //16000 和 8000;
    [self.synthesizer setParameter:@"tts.pcm"
  ...
  if (self.callbackId) {
    NSDictionary *info =
        [NSDictionary dictionaryWithObjectsAndKeys:
                          @"SpeechError", STR_EVENT,
                          [NSNumber numberWithInt:errorCode.errorCode],
                          STR_CODE, errorCode.errorDesc, STR_MESSAGE, nil];
...
- (void)onResults:(NSArray *)results isLast:(BOOL)isLast {
  ...
  if (self.callbackId) {
    ...
    NSDictionary *info =
        [NSDictionary dictionaryWithObjectsAndKeys:@"SpeechResults", STR_EVENT,
                                                   text, STR_RESULTS, nil];
    CDVPluginResult *result =
        [CDVPluginResult resultWithStatus:CDVCommandStatus_OK
                      messageAsDictionary:info];
    [result setKeepCallbackAsBool:YES];
...
- (void)onVolumeChanged:(int)volume {
  ...
  if (self.callbackId) {
    NSDictionary *info = [NSDictionary
        dictionaryWithObjectsAndKeys:@"VolumeChanged", STR_EVENT,
                                     [NSNumber numberWithInt:volume],
                                     STR_VOLUME, nil];
    CDVPluginResult *result =
        [CDVPluginResult resultWithStatus:CDVCommandStatus_OK
                      messageAsDictionary:info];
...
- (void)onCompleted:(IFlySpeechError *)error {
  ...
  if (self.callbackId) {
    NSDictionary *info = [NSDictionary
        dictionaryWithObjectsAndKeys:@"SpeakCompleted", STR_EVENT,
                                     [NSNumber numberWithInt:error.errorCode],
                                     STR_CODE, error.errorDesc, STR_MESSAGE,
                                     nil];
    CDVPluginResult *result =
        [CDVPluginResult resultWithStatus:CDVCommandStatus_OK
                      messageAsDictionary:info];
...
- (void)onBufferProgress:(int)progress message:(NSString *)msg {
  ...
  if (self.callbackId) {
    NSDictionary *info = [NSDictionary
        dictionaryWithObjectsAndKeys:@"BufferProgress", STR_EVENT,
                                     [NSNumber numberWithInt:progress],
                                     STR_PROGRESS, msg, STR_MESSAGE, nil];
    CDVPluginResult *result =
        [CDVPluginResult resultWithStatus:CDVCommandStatus_OK
                      messageAsDictionary:info];
    [result setKeepCallbackAsBool:YES];
...
- (void)onSpeakProgress:(int)progress {
  ...
  if (self.callbackId) {
    NSDictionary *info = [NSDictionary
        dictionaryWithObjectsAndKeys:@"SpeakProgress", STR_EVENT,
                                     [NSNumber numberWithInt:progress],
                                     STR_PROGRESS, nil];
    CDVPluginResult *result =
        [CDVPluginResult resultWithStatus:CDVCommandStatus_OK
...
- (void)fireEvent:(NSString *)event {
  if (self.callbackId) {
    NSDictionary *info =
        [NSDictionary dictionaryWithObject:event forKey:STR_EVENT];
    CDVPluginResult *result =
        [CDVPluginResult resultWithStatus:CDVCommandStatus_OK
                      messageAsDictionary:info];
    [result setKeepCallbackAsBool:YES];
...
- (void)playVoice:(NSString *)voiceName {
  if (self.mymedia.playing) {
    [self.mymedia stop];
    return;
  }
  NSString *path = [[NSString alloc] init];
  AVAudioPlayer *player = [[AVAudioPlayer alloc] init];
  NSString *voicePath = [@"/www/audio/" stringByAppendingString:voiceName];
  path = [[NSBundle mainBundle] pathForResource:voicePath ofType:@"wav"];
  NSData *fileData = [NSData dataWithContentsOfFile:path];
  NSError *myerror = [[NSError alloc] init];
  player = [[AVAudioPlayer alloc] initWithData:fileData
                                  fileTypeHint:AVFileTypeAC3
                                         error:&myerror];
  NSLog(@"%@", myerror);
  self.mymedia = player;
  [self.mymedia play];
